...
## READS
- `clients/audaura-unitar/assets/build_ads_dashboard.py` — the renderer (never edit output HTML by hand)
- `clients/audaura-unitar/assets/meta_ads_snapshot.json` — the data the renderer consumes
- Meta Ads account **1034316391892752** ("UNITAR (MYR) - Audaura", currency MYR)

## WRITES
- `clients/audaura-unitar/assets/meta_ads_snapshot.json` (fresh pull)
- `clients/audaura-unitar/assets/creatives/<ad_id>.jpg` (downloaded thumbnails, white-label safe)
...
### 3. Build
`python3 clients/audaura-unitar/assets/build_ads_dashboard.py`
...
All three scripts live in `clients/audaura-unitar/assets/`:
...
2. `set -a; source tools/meta-ads-cli/.env; set +a` then `export AD_ACCOUNT_ID=act_1034316391892752` and run `python3 clients/audaura-unitar/assets/pull_meta.py` once; verify leads/spend match, fix `LEAD_ACTIONS` if off.
