---
client: kynare
title: Lead gen form and WhatsApp greeting copy — Meta and TikTok, EN and BM
date: 2026-08-12
product: PT Hybrid program
status: drafted, pending client sign-off and a WhatsApp number
last_reviewed: 2026-08-12
---

# Kynare — lead form and WhatsApp copy (PT Hybrid program)

Covers the Meta Instant Lead Gen Form, the Meta WhatsApp greeting template, and the TikTok Instant Lead Gen Form, in English and Bahasa Malaysia.

> **Client feedback applied 2026-08-12.** The client asked for **"Human Performance Medicine"** as the form headline on both platforms, with the description explaining what it consists of. They also asked us to drop "physio and PT under one roof", because plenty of clinics claim it and deliver poorly. Both changes are in below. The term stays in English on the Malay forms so it holds as a brand term. Two cautions on the word "Medicine" are recorded in `../../style-pack.md`.

> **No verified proof exists for this client.** As of 2026-08-12 there is no testimonial, review count or outcome statistic available (site is JavaScript-rendered and returns nothing, no listings indexed). Every value line below is built from structurally true facts. Do not substitute an invented number. The testimonial slot at the bottom shows where a real quote goes once the client supplies one.

---

## 1. Meta — English lead gen form (Higher Intent)

### Intro

| Field | Copy |
|---|---|
| Headline | Human Performance Medicine |
| Alt headline to test | Human Performance Medicine at Kynare. |

**Description**

> Human Performance Medicine looks at the whole system behind the pain. It brings together physical therapy, strength and movement calibration, and diagnostics such as blood and biomarker analysis.
>
> Every client starts with the KYNARE Outset, a one to one assessment. We explain what is causing the problem and show you the plan before you commit.

### Ending (Message for Leads)

| Field | Copy |
|---|---|
| Headline | Thank you. Your request is with our team. |
| Description | Our team will WhatsApp you during opening hours, Monday to Sunday, 8am to 7pm, to confirm your time and your clinic, PJ Section 13 or Mont Kiara. You can also message us now using the button below. |
| Additional Action | Contact us on WhatsApp |
| Link | `https://wa.me/60XXXXXXXXX` — **pending the clinic's WhatsApp number** |
| CTA | Contact us on WhatsApp |

---

## 2. Meta — English FB/IG/WhatsApp greeting (Start Conversations)

**Option A (recommended)**

> Hi! Thanks for messaging Kynare. Our approach is Human Performance Medicine, which brings together physical therapy, strength and movement calibration, and diagnostics such as blood and biomarker analysis.
>
> Every client starts with a one to one assessment, so we can explain what is causing the problem before you decide on any program.
>
> To arrange your appointment, send us the following details and your interested service in one message:
> 1. Your full name
> 2. Your email
> 3. Your phone number
> 4. Interested service
>
> Our team will confirm your appointment from there.

**Option B (tighter opening, same list and closing as Option A)**

> Hi! Thanks for messaging Kynare. Our approach is Human Performance Medicine: physical therapy, strength and movement calibration, and diagnostics working together. Every client starts with a one to one assessment so we can explain what is causing the problem before you decide on any program.

---

## 3. Meta — Bahasa Malaysia lead gen form (Higher Intent)

### Intro

| Field | Copy |
|---|---|
| Headline | Human Performance Medicine |
| Alt headline to test | Human Performance Medicine di Kynare. |

**Description**

> Human Performance Medicine melihat keseluruhan sistem di sebalik kesakitan anda. Ia menggabungkan terapi fizikal, kalibrasi kekuatan dan pergerakan, serta diagnostik seperti analisis darah dan biomarker.
>
> Setiap klien bermula dengan KYNARE Outset, sesi pemeriksaan satu ke satu. Kami terangkan punca masalah dan tunjukkan pelan anda sebelum anda membuat keputusan.

### Ending (Message for Leads)

| Field | Copy |
|---|---|
| Headline | Terima kasih. Maklumat anda sudah kami terima. |
| Description | Pasukan kami akan WhatsApp anda pada waktu operasi, Isnin hingga Ahad, 8 pagi hingga 7 petang, untuk mengesahkan masa dan klinik anda, PJ Seksyen 13 atau Mont Kiara. Anda juga boleh hubungi kami sekarang melalui butang di bawah. |
| Additional Action | Hubungi kami di WhatsApp |
| Link | `https://wa.me/60XXXXXXXXX` |
| CTA | Hubungi kami di WhatsApp |

---

## 4. Meta — Bahasa Malaysia FB/IG/WhatsApp greeting (Start Conversations)

**Option A (recommended)**

> Hai! Terima kasih kerana menghubungi Kynare. Pendekatan kami ialah Human Performance Medicine, yang menggabungkan terapi fizikal, kalibrasi kekuatan dan pergerakan, serta diagnostik seperti analisis darah dan biomarker.
>
> Setiap klien bermula dengan sesi pemeriksaan satu ke satu, supaya kami boleh terangkan punca sebenar masalah anda sebelum anda memilih sebarang program.
>
> Untuk menetapkan temujanji, sila hantar maklumat berikut beserta perkhidmatan yang diminati dalam satu mesej:
> 1. Nama penuh anda
> 2. Emel anda
> 3. Nombor telefon anda
> 4. Perkhidmatan yang diminati
>
> Pasukan kami akan mengesahkan temujanji anda selepas itu.

**Option B (tighter opening, same list and closing as Option A)**

> Hai! Terima kasih kerana menghubungi Kynare. Pendekatan kami ialah Human Performance Medicine: terapi fizikal, kalibrasi kekuatan dan pergerakan, serta diagnostik yang bekerja bersama. Setiap klien bermula dengan sesi pemeriksaan satu ke satu, supaya kami boleh terangkan punca sebenar masalah anda sebelum anda memilih sebarang program.

---

## 5. TikTok — English lead gen form (Higher Intent)

TikTok runs broad with no interest targeting, so the reader arrives cold and may not know what Kynare is. The clinic and locations go back into the description here, unlike the Meta version where interest targeting pre-qualifies. TikTok forms are also read fast in a vertical sheet and follow a Spark Ad, so shorter and plainer works better.

### Intro

| Field | Copy |
|---|---|
| Headline | Human Performance Medicine |
| Alt headline to test (recommended on TikTok) | Human Performance Medicine in PJ and Mont Kiara. |

**Description**

> Human Performance Medicine at Kynare, in PJ Section 13 and Mont Kiara. It brings together physical therapy, strength and movement calibration, and diagnostics such as blood and biomarker analysis.
>
> Every client starts with the KYNARE Outset, a one to one assessment. Leave your details and our team will WhatsApp you to set a time.

### Ending (client-authored, unchanged)

| Field | Copy |
|---|---|
| Headline | Thank you! We'll be in touch with you shortly. |
| Description | Our team from Kynare will WhatsApp you as soon as possible during our working hours, Monday to Sunday, 8am to 7pm. Do visit our website for more details |
| Additional Action | View Website |
| Link | `https://www.kynare.com/class` |
| CTA | View Website |

---

## 6. TikTok — Bahasa Malaysia lead gen form (Higher Intent)

### Intro

| Field | Copy |
|---|---|
| Headline | Human Performance Medicine |
| Alt headline to test (recommended on TikTok) | Human Performance Medicine di PJ dan Mont Kiara. |

**Description**

> Human Performance Medicine di Kynare, PJ Seksyen 13 dan Mont Kiara. Ia menggabungkan terapi fizikal, kalibrasi kekuatan dan pergerakan, serta diagnostik seperti analisis darah dan biomarker.
>
> Setiap klien bermula dengan KYNARE Outset, sesi pemeriksaan satu ke satu. Tinggalkan maklumat anda dan pasukan kami akan WhatsApp untuk tetapkan masa.

### Ending (client-authored, unchanged)

| Field | Copy |
|---|---|
| Headline | Terima kasih! Kami akan hubungi anda tidak lama lagi |
| Description | Kami akan WhatsApp secepat mungkin pada waktu bekerja, Isnin hingga Ahad, 8 pagi hingga 7 petang. Lawati laman web kami untuk butiran lanjut |
| Additional Action | Lawati Laman Web |
| Link | `https://www.kynare.com/class` |
| CTA | Lawati Laman Web (**currently reads "View Website" in the sheet, fix so the Malay form is fully Malay**) |

---

## Where a real testimonial goes

Once the client supplies a quote with a first name and a rough timeframe, add it after the Human Performance Medicine line in either greeting. Keep the term, since it is the positioning the client asked for, and let the quote carry the proof.

**English**

> Hi! Thanks for messaging Kynare. Our approach is Human Performance Medicine, which brings together physical therapy, strength and movement calibration, and diagnostics such as blood and biomarker analysis.
>
> "[exact client quote]" is how [First name, age] described it after [number] weeks with us. Every client starts the same way, with a one to one assessment.

**Bahasa Malaysia**

> Hai! Terima kasih kerana menghubungi Kynare. Pendekatan kami ialah Human Performance Medicine, yang menggabungkan terapi fizikal, kalibrasi kekuatan dan pergerakan, serta diagnostik seperti analisis darah dan biomarker.
>
> "[petikan klien]" itu kata [Nama, umur] selepas [bilangan] minggu bersama kami. Setiap klien bermula dengan cara yang sama, iaitu sesi pemeriksaan satu ke satu.

---

## Blockers on these forms

1. **The privacy policy field on both platforms points to `https://www.kynare.com/contact-us`,** which is a contact page and not a privacy policy. Meta and TikTok both require a genuine privacy policy URL, and this is a common rejection reason. The page needs to exist before the forms go for review.
2. **The WhatsApp number for the Meta `wa.me` link is still outstanding.**
3. **Confirm `kynare.com/class` is the right landing spot** for someone who just asked about the PT Hybrid program, since every TikTok lead is sent there after submitting.
4. **Keep ad creative clear of second-person health assertions.** Meta's personal attributes policy blocks ads implying we know the reader's condition. The form copy above is deliberately conditional and general. This matters more in the ad creative than in the form.
